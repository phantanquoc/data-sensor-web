...
let id_document_plc_2;

exports.postDataPlc_to_noi_chien_2 = async (
  values,
  io_,
  Start_PLC2,
  giai_doan_1,
  ...
  // khởi tạo
  if (Start_PLC2 === 1) {
    const docunent = await PLCData_noi_chien_2.create(dataFormat).catch(
      (err) => {
        console.log(err);
      },
    );
    if (docunent) id_document_plc_2 = docunent._id;
  }
  // update
  if (Start_PLC2 > 1) {
    ...
    if (giai_doan_1 && typeof giai_doan_1 === "boolean") {
      const document = await PLCData_noi_chien_2.findByIdAndUpdate(
        id_document_plc_2,
        {
