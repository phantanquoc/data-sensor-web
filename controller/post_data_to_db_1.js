...
let id_document_plc_1;

exports.postDataPlc_to_noi_chien_1 = async (
  values,
  io_,
  Start_PLC1,
  giai_doan_1,
  ...
  // khởi tạo
  if (Start_PLC1 === 1) {
    const docunent = await PLCData_noi_chien_1.create(dataFormat).catch(
      (err) => {
        console.log(err);
      },
    );
    if (docunent) id_document_plc_1 = docunent._id;
  }
  // update
  if (Start_PLC1 > 1) {
    ...
    if (giai_doan_1 && typeof giai_doan_1 === "boolean") {
      const document = await PLCData_noi_chien_1.findByIdAndUpdate(
        id_document_plc_1,
        {
