...
let id_document_plc_5;

exports.postDataPlc_to_noi_chien_5 = async (
  values,
  io_,
  Start_PLC5,
  giai_doan_1,
  ...
  // khởi tạo
  if (Start_PLC5 === 1) {
    const docunent = await PLCData_noi_chien_5.create(dataFormat).catch(
      (err) => {
        console.log(err);
      },
    );
    if (docunent) id_document_plc_5 = docunent._id;
  }
  // update
  if (Start_PLC5 > 1) {
    ...
    if (giai_doan_1 && typeof giai_doan_1 === "boolean") {
      const document = await PLCData_noi_chien_5.findByIdAndUpdate(
        id_document_plc_5,
        {
