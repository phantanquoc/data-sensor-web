...
let id_document_plc_7;

exports.postDataPlc_to_noi_chien_7 = async (
  values,
  io_,
  Start_PLC7,
  giai_doan_1,
  ...
  // khởi tạo
  if (Start_PLC7 === 1) {
    const docunent = await PLCData_noi_chien_7.create(dataFormat).catch(
      (err) => {
        console.log(err);
      },
    );
    if (docunent) id_document_plc_7 = docunent._id;
  }
  // update
  if (Start_PLC7 > 1) {
    ...
    if (giai_doan_1 && typeof giai_doan_1 === "boolean") {
      const document = await PLCData_noi_chien_7.findByIdAndUpdate(
        id_document_plc_7,
        {
