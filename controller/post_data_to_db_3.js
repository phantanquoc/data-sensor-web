...
let id_document_plc_3;

exports.postDataPlc_to_noi_chien_3 = async (
  values,
  io_,
  Start_PLC3,
  giai_doan_1,
  ...
  // khởi tạo
  if (Start_PLC3 === 1) {
    const docunent = await PLCData_noi_chien_3.create(dataFormat).catch(
      (err) => {
        console.log(err);
      },
    );
    if (docunent) id_document_plc_3 = docunent._id;
  }
  // update
  if (Start_PLC3 > 1) {
    ...
    if (giai_doan_1 && typeof giai_doan_1 === "boolean") {
      const document = await PLCData_noi_chien_3.findByIdAndUpdate(
        id_document_plc_3,
        {
