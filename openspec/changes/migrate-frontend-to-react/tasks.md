...
- [x] 5.2 Port donut timer state (`stageTimer {stage,startMs,targetMin}` via ref) + `seedStartMsFromDoc` (computes `elapsedAtSeed = tsLast - tsFirst` from `bien_du_lieu[1]` to `bien_du_lieu[last]`, clamped >= 0, then returns `startMs = Date.now() - elapsedAtSeed`; this seeds the donut so displayed elapsed = the DB span of the stage's recorded data, frozen when data stops arriving; returns null when doc can't provide a seed, caller falls back to Date.now()). Seed-then-LIVE transition, stage-transition clearing, gd1-3 target from `thoi_gian_chay` / gd4 from `thoi_gian_treo_long` ← (verify: elapsedAtSeed = tsLast - tsFirst; startMs = Date.now() - elapsedAtSeed; null fallback works; only active stage shows donut)
