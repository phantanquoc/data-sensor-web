...
Replicate `stageTimer { stage, startMs, targetMin }` in a ref, and the 1s `setInterval` in a `useEffect`. `renderDonut` becomes JSX/SVG: radius=27, circumference=169.646, stroke `#00aaff` normally / `#fd7e14` when `elapsedMin > targetMin`, center text `"<elapsed.toFixed(1)> / <round(target)>"` over `"phút"`. `seedStartMsFromDoc(doc, gNum)` computes `elapsedAtSeed = tsLast - tsFirst` (from `bien_du_lieu[1]` to `bien_du_lieu[last]`, both parsed via `parseTs("HH:MM:SS DD/MM/YYYY")`, skipping [0] init record, clamped >= 0) and returns `startMs = Date.now() - elapsedAtSeed`. This seeds the donut so that displayed elapsed = the DB span of the stage's recorded data — if data stops arriving (HMI disconnect, batch ended), the elapsed time freezes at the last known span rather than counting wall-clock forward (matching EJS behavior). Returns null when the doc can't provide a seed (insufficient entries or unparseable timestamps); the caller then falls back to `Date.now()` for a stage that only just went active live. After first seed the live timer counts forward from the seeded point. gd1-3 `targetMin` from `set_giai_doan.thoi_gian_chay`, gd4 from `thoi_gian_treo_long`. Only the currently active stage shows a donut; stage transitions clear the previous donut. Alternative (CSS-only animation) rejected: cannot reproduce the exact seeded-elapsed math.

**Decision: `useFryerData` hook owns per-fryer realtime state; `auto_load` ported into it.**
On tab change: reset the realtime view, `socket.emit("join_noi", n)`, then run the `auto_load_noi_chien` sequence — `GET /get_noi_chien` (returns newest-first) → forward scan for the first batch with empty `thoi_gian_stop`, fallback to newest (`documents[0]`) → `GET /get_noi_chien_detail` → rebuild 4 stage payloads (`active:false`, `data` = last element of each `bien_du_lieu`) and feed them through the same render path as socket ticks. Store `currentRunningDoc` so the donut seeds on the first live tick. This preserves the exact "instant snapshot without waiting for socket" behavior.
...
- **[Donut math diverges from the original]** → `seedStartMsFromDoc` computes `elapsedAtSeed = tsLast - tsFirst` and returns `startMs = Date.now() - elapsedAtSeed` (matching EJS exactly); verify against a running batch that displayed elapsed = DB span and freezes when data stops arriving.
