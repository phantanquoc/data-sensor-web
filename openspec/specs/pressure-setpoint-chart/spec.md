# pressure-setpoint-chart Specification

## Purpose
Deriving a per-machine target-pressure series from the stored per-machine configuration and that machine's measured pressure points, and rendering it as a dashed reference line on the vacuum pressure chart of the per-machine detail page.
## Requirements
